...
- **Registro de Abastecimiento:** Flujo móvil-first optimizado para el bombero de turno. Exige declaración diaria obligatoria de apertura de tanques, captura del conductor responsable y foto obligatoria del dispensador/surtidor. La **foto del odómetro es opcional** — su ausencia no bloquea el guardado del formulario, pero su presencia activa el OCR de IA para autocompletar el kilometraje. Ambos campos de foto abren un **selector explícito de origen** (hoja inferior táctil) con dos opciones: "Tomar Foto", que fuerza la cámara trasera mediante `capture="environment"`, y "Subir de Galería", que abre el explorador de imágenes del dispositivo. No se delega la decisión al diálogo nativo de `accept="image/*"`, cuyo comportamiento es inconsistente entre Android, iOS y la app instalada como PWA (en varios casos abre directamente una sola fuente sin preguntar). Cada origen usa su propio `<input type="file">` y se reinicia su `value` tras la selección, de modo que volver a elegir el mismo archivo dispara el `onChange` de nuevo.
...
- **Prohibición de Valores por Defecto en Litros (Integridad del Inventario):** El campo "Litros a Surtir" **nunca se precarga con un valor**. Sólo puede poblarse desde el OCR de la foto del dispensador o desde el teclado del bombero. Anteriormente se autocompletaba con la capacidad del tanque del vehículo (y con la cantidad aprobada en los suministros por bypass), lo que constituía un riesgo directo de integridad: si la IA fallaba o devolvía nulo y el bombero no verificaba el campo, el suministro se guardaba con esa cifra plausible pero nunca confirmada, **descontando del tanque combustible que no se surtió**. La capacidad del vehículo y el tope aprobado se comunican ahora sólo como `placeholder` del input y como su atributo `max`, siguiendo el mismo criterio del campo de kilometraje (que muestra el último odómetro conocido sin escribirlo). Con el campo vacío, la validación de envío (`litros > 0`) falla de forma cerrada y obliga a una cifra deliberada.
